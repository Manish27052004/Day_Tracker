-- ============================================================================
-- DIAGNOSTIC STREAK SYSTEM - Debug Version
-- ============================================================================
-- This version adds extensive logging to see WHY yesterday is not found
-- ============================================================================
...
BEGIN
    ...
    -- If no yesterday found, let's see what dates DO exist
    IF yesterday_count = 0 THEN
        RAISE NOTICE '   ⚠️ NO YESTERDAY FOUND! Checking what dates exist...';
    ...
    -- Fetch yesterday's data (if exists)
    SELECT 
    ...
    RAISE NOTICE '   Yesterday data retrieved:';
    RAISE NOTICE '      Achiever strike: %', prev_achiever;
    RAISE NOTICE '      Fighter strike: %', prev_fighter;
    RAISE NOTICE '      Progress: %', prev_progress;

    -- Validate the chain
    IF prev_progress < min_pct THEN
        RAISE NOTICE '   ⚠️ Yesterday failed (% < %%), resetting achiever to 0', prev_progress, min_pct;
        prev_achiever := 0;
    ELSE
    ...
    IF prev_progress <= 100 THEN
        RAISE NOTICE '   Yesterday did not exceed 100%% (% <= 100), resetting fighter to 0', prev_progress;
        prev_fighter := 0;
    ELSE
    ...
    ELSE 
        NEW.fighter_strike := 0;
        RAISE NOTICE '   Today not fighter (% <= 100), strike = 0', NEW.progress;
    END IF;

    RAISE NOTICE '🎯 FINAL: Achiever=%, Fighter=%', NEW.achiever_strike, NEW.fighter_strike;
    RAISE NOTICE '====================================';
...
RAISE NOTICE '';
RAISE NOTICE '✅ DIAGNOSTIC VERSION INSTALLED';
RAISE NOTICE '';
RAISE NOTICE '📝 Now try creating/updating a task and check the NOTICE messages';
RAISE NOTICE '   They will tell you EXACTLY why yesterday is/isn''t found';
RAISE NOTICE '';
RAISE NOTICE '💡 To see messages:';
RAISE NOTICE '   - In Supabase SQL Editor: They appear in the output panel';
RAISE NOTICE '   - In psql: They show in the console';
RAISE NOTICE '';
